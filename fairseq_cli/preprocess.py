...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, append_eos, num_workers):
    ...
    def make_dataset(vocab, input_prefix, output_prefix, lang, append_eos, num_workers=1):
        if args.dataset_impl == "raw":
            ...
            make_binary_dataset(vocab, input_prefix, output_prefix, lang, append_eos, num_workers)

    def make_all(lang, vocab, append_eos):
        if args.trainpref:
            make_dataset(vocab, args.trainpref, "train", lang, append_eos, num_workers=args.workers)
        if args.validpref:
            for k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "valid{}".format(k) if k > 0 else "valid"
                make_dataset(vocab, validpref, outprefix, lang, append_eos, num_workers=args.workers)
        if args.testpref:
            for k, testpref in enumerate(args.testpref.split(",")):
                outprefix = "test{}".format(k) if k > 0 else "test"
                make_dataset(vocab, testpref, outprefix, lang, append_eos, num_workers=args.workers)
    ...
    make_all(args.source_lang, src_dict, not args.no_src_eos)
    if target:
        make_all(args.target_lang, tgt_dict, not args.no_tgt_eos)
